...
export default function HomePage() {
  ...
  if (selectedTag) {
    filteredCourses = filteredCourses.filter((course) => course.tags.includes(selectedTag));
  }
  ...
  if (searchQuery.trim()) {
    ...
    filteredCourses = filteredCourses.filter(
      (course) =>
        ...
        course.prophet.toLowerCase().includes(query)
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-4 mb-10">
      ...
      {filteredCourses.length > COURSES_PER_PAGE && (
        ...
        <p className="text-center text-sm text-zinc-500 mt-4">
          Page {page + 1} of {Math.ceil(filteredCourses.length / COURSES_PER_PAGE)}
        </p>
      </div>
