...
func (c *notificationConsumer) handleOrderCompleted(ctx context.Context, delivery amqp.Delivery) error {
	...
	messageID, err := c.chatService.SaveMessage(ctx, orderCompletedData.RoomID, "system", content, string(contract.OrderCompletedEvent))
	if err != nil {
...
func (c *notificationConsumer) handleOrderPaymentBound(ctx context.Context, delivery amqp.Delivery) error {
	...
	messageID, err := c.chatService.SaveMessage(ctx, orderPaymentBoundData.RoomID, "system", content, string(contract.OrderPaymentBoundEvent))
	if err != nil {
		log.Printf("Failed to save message: %v", err)
		return err
	}
...
func (c *notificationConsumer) handleOrderPaid(ctx context.Context, delivery amqp.Delivery) error {
	...
	messageID, err := c.chatService.SaveMessage(ctx, orderPaidData.RoomID, "system", content, string(contract.OrderPaidEvent))
	if err != nil {
		log.Printf("Failed to save message: %v", err)
		return err
	}
